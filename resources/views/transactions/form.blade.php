...
@section('content')
    <div class="card" style="background: var(--dark-light); border-radius: 15px; padding: 40px; box-shadow: 0 10px 30px rgba(0,0,0,0.5); max-width: 900px; margin: 0 auto;">
        ...
        <form method="POST" action="{{ route('transactions.store') }}">
            ...
            <!-- Campo oculto para enviar la referencia generada automáticamente -->
            <input type="hidden" name="reference" id="reference-hidden">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px; margin-bottom: 40px;">
                ...
                <div id="casilla-contrato" style="background: rgba(255,102,0,0.12); border-radius: 15px; padding: 20px; text-align: center; display: none; border: 1px solid rgba(255,102,0,0.3);">
                    ...
                    <div id="contrato-info" style="color: var(--primary); font-weight: bold; font-size: 15px; margin-bottom: 12px; line-height: 1.4;">—</div>
                ...
                <div id="casilla-saldo" style="background: rgba(255,255,255,0.08); border-radius: 15px; padding: 20px; text-align: center; display: none; border: 1px solid rgba(255,255,255,0.2);">
                    ...
                    <div id="mensaje-saldo" style="color: #ff9; font-size: 16px; font-weight: bold;">—</div>
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px; margin-bottom: 30px;">
                <div>
                    ...
                    <input type="date" name="date" id="payment-date" value="{{ old('date', now()->format('Y-m-d')) }}" required 
                ...
                <div>
                    ...
                    <select name="payment_method" id="payment-method-select" required style="
            ...
            <!-- Referencia (automática) y Notas -->
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px; margin-bottom: 40px;">
                <div>
                    <label class="field-label" style="color: var(--primary); font-weight: bold;">Referencia / Operación</label>
                    <div id="reference-display" style="width: 100%; padding: 16px; border-radius: 15px; background: rgba(255,255,255,0.1); border: none; color: #ccc; font-size: 16px; text-align: center; font-weight: bold;">
                        Generando...
                    </div>
                    <small style="color: #aaa; font-size: 13px; margin-top: 6px; display: block;">
                        Código automático generado por el sistema (no editable)
                    </small>
                </div>
    ...
    <!-- JavaScript (con generación automática de referencia) -->
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const select = document.getElementById('contract-select');
            const amountInput = document.getElementById('amount-input');
            const paymentDate = document.getElementById('payment-date');
            const referenceHidden = document.getElementById('reference-hidden');
            ...
            // Función para generar código automático OP-YYYYMMDD-NNN
            const generateReference = () => {
                const date = new Date(paymentDate.value || new Date());
                const year = date.getFullYear();
                const month = String(date.getMonth() + 1).padStart(2, '0');
                const day = String(date.getDate()).padStart(2, '0');
                const dateStr = `${year}${month}${day}`;

                // Número secuencial: usa un contador simple por día (puedes mejorarlo con BD más adelante)
                // Por ahora, usamos un número random o incremental simple (ej: 001, 002...)
                const randomNum = Math.floor(Math.random() * 999) + 1; // 001-999
                const seq = String(randomNum).padStart(3, '0');

                const code = `OP-${dateStr}-${seq}`;
                referenceHidden.value = code;
                referenceDisplay.textContent = code;
            };

            // Actualización de deuda y saldo
            const update = () => {
                ...
                mensajeSaldo.innerHTML = nuevoPendiente === 0 
                    ? '<span style="color:#0f8;">¡Contrato pagado completo!</span>'
                    : 'Quedará pendiente';
            ...
            paymentDate.addEventListener('change', generateReference); // regenera si cambian la fecha

            // Inicializar
            update();
            generateReference(); // Genera al cargar
        });
    </script>
@endsection