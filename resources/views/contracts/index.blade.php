...
@section('content')
    <div class="card" style="background: var(--dark-light); border-radius: 15px; padding: 30px; box-shadow: 0 10px 30px rgba(0,0,0,0.5);">
        <div class="header-table" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; flex-wrap: wrap; gap: 20px;">
            <h2 style="color: var(--primary); font-size: 32px;">Lista de Contratos</h2>
            <a href="{{ route('contracts.create') }}" class="btn-login" style="
            display: inline-flex;
            align-items: center;
            gap: 8px;
            padding: 12px 25px;
        ...
        <form method="GET" action="{{ route('contracts.index') }}" style="margin-bottom: 30px;">
            <div style="display: flex; gap: 15px; flex-wrap: wrap;">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Buscar por código o cliente..." 
                ...
                <select name="status" style="padding: 14px; border-radius: 15px; background: rgba(255,255,255,0.1); border: none; color: white; font-size: 16px;">
                    ...
                    <option value="en_produccion" {{ request('status') == 'en_produccion' ? 'selected' : '' }}>En Producción</option>
                ...
                @if(request('search') || request('status'))
                    <a href="{{ route('contracts.index') }}" class="btn-login" style="background: #666; padding: 14px 20px;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <thead>
                    <tr style="background: var(--gray);">
                        ...
                        <th style="padding: 15px; text-align: left; color: var(--primary);">Cantidad</th>
                ...
                <tbody>
                    @forelse($contracts as $contract)
                        <tr style="border-bottom: 1px solid rgba(255,102,0,0.2);">
                            ...
                            <td style="padding: 15px; color: white;">{{ $contract->person->getFullNameAttribute() }}</td>
                            ...
                            <td style="padding: 15px; color: white;">{{ $contract->quantity }} m³</td>
                            ...
                            <td style="padding: 15px; text-align: center;">
                                <a href="{{ route('contracts.edit', $contract) }}" style="color: var(--primary); margin: 0 10px;">
